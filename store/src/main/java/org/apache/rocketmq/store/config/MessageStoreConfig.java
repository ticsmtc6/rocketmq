...
public class MessageStoreConfig {
     ...
     * 清除过期文件线程在初次销毁mappedfile时，如果该文件被其他线程引用，
     * 引用次数大于0.则设置MappedFile的可用状态为false，
     * 并设置第一次删除时间，下一次清理任务到达时，
     * 如果系统时间大于初次删除时间加上本参数，则将ref次数一次减1000，
     * 知道引用次数小于0，则释放物理资源
     */
    ...
    // 文件保留时间，默认72小时，表示当前写文件最后一次更新时间加上filereservedtime小与当前时间，该文件将被清理
    ...
    // ConsumeQueue两次刷盘的最大间隔,如果超过该间隔,将忽略
    ...
    // 允许从服务器落后的最大偏移字节数,默认为256M。超过该值则表示该Slave不可用
